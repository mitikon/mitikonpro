"""Guarded ingestion foundation for public JRA pages used by racing 先行予測λ.

Important:
- This module is for the horse-racing 先行予測λ only.
- It is NOT the 部分空間正則化PCA / 先行シグナル予測λ system.
...
def ingest_snapshot(
    *,
    ...
    ``payload`` must contain only information actually visible/known at
    ``observed_at``.  For PRE_RACE this means no finish/result fields.
    """
...
def freeze_snapshot(snapshot: OfficialSnapshot, root: str | Path) -> Path:
    ...
    if target.exists():
        return target
    # Exclusive creation prevents accidental post-result rewriting.
